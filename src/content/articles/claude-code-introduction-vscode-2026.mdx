...
flowchart LR
  ...
  classDef core fill:#c9a64a,stroke:#c9a64a,color:#0a0e16,stroke-width:0px,font-weight:700
  classDef user fill:#0a0e16,stroke:#cfcfcf,color:#f5f5f5,stroke-width:1.5px,font-weight:600
  classDef ext fill:#1e2a3a,stroke:#c9a64a,color:#f5f5f5,stroke-width:1.5px,font-weight:500

  class ClaudeCode core
  class User user
  class Files,Term,Ext,Memory,Sub ext

  linkStyle default stroke:#c9a64a,stroke-width:1.5px
```
...
```mermaid
flowchart LR
  CC([Claude Code]) -->|形態| CCForm[CLI + VS Code 拡張]
  CC -->|粒度| CCGrain[業務委譲型 / 自律実行]
  Cursor([Cursor]) -->|形態| CursorForm[専用 IDE / VS Code フォーク]
  Cursor -->|粒度| CursorGrain[IDE 内補完・編集型]
  Copilot([GitHub Copilot]) -->|形態| CopilotForm[VS Code 拡張]
  Copilot -->|粒度| CopilotGrain[インライン補完中心]
  AntiG([Google Antigravity]) -->|形態| AntiForm[専用エディタ]
  AntiG -->|粒度| AntiGrain[Gemini 3 統合・委譲型]

  classDef tool fill:#c9a64a,stroke:#c9a64a,color:#0a0e16,stroke-width:0px,font-weight:700
  classDef attr fill:#1e2a3a,stroke:#cfcfcf,color:#f5f5f5,stroke-width:1px,font-weight:500

  class CC,Cursor,Copilot,AntiG tool
  class CCForm,CursorForm,CopilotForm,AntiForm,CCGrain,CursorGrain,CopilotGrain,AntiGrain attr

  linkStyle default stroke:#c9a64a,stroke-width:1.2px
```
